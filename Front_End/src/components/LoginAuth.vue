      ...
      <div class="illustration-section">
        ...
        <div class="illustration-container">
          ...
          <div class="user-avatar">
            <img src="@/assets/1x/1x/Asset 4.png" alt="User Avatar" class="avatar-image" />
          </div>
      </div>

        <div class="illustration-text">
          <h1>{{ t.welcomeToSeoTools }}</h1>
          <p>{{ t.signInSecurely }}</p>
        </div>
      </div>
      ...
      <div class="login-form-section">
        <div class="login-card">
          ...
          <div class="login-footer">
            <p>{{ t.newToSeoTools }} 
              <router-link to="/register" class="register-link">{{ t.createAccount }}</router-link>
            </p>
          </div>
        </div>
      </div>
...
const valid = computed(() => {
  return identifier.value && password.value && identifier.value.includes('@');
});
...
const submitForm = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      router.push({ name: 'BarcodeSearch' });
    }
  } catch (error) {
    if (error.response && error.response.status === 401) {
      loginError.value = true;
    }
    console.error(t.value.loginError, error.response?.data?.detail);
  } finally {
    loading.value = false;
  }
};
